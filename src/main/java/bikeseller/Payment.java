...
public class Payment {
    ...
    public void onPreUpdate(){
        if("OrderCancelled".equals(process)) {
            ...
            bikeseller.external.Promotion promotion = new bikeseller.external.Promotion();
            promotion.setOrderId(getOrderId());
            promotion.setPoint((double)-1);
            promotion.setProcess("PayCancelled");
            // mappings goes here
            PayApplication.applicationContext.getBean(bikeseller.external.PromotionService.class)
                    .payCancel(promotion);
